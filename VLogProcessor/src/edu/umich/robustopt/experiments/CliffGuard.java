...
import edu.umich.robustopt.postgres.PostgresDatabaseLoginConfiguration;
import edu.umich.robustopt.postgres.PostgresQueryPlanParser;
import edu.umich.robustopt.postgres.PostgresDeployer;
import edu.umich.robustopt.postgres.PostgresDesigner;
import edu.umich.robustopt.postgres.PostgresLatencyMeter;
...
public class CliffGuard {
	...
	private static String performDesign(DatabaseLoginConfiguration emptyDB, DatabaseLoginConfiguration fullDB, 
		...
		} else if (emptyDB instanceof PostgresDatabaseLoginConfiguration){
			System.out.print("START POSTGRES DESIGN STUFF\n");
			if (experimentCache==null)
				experimentCache = new ExperimentCache(cacheFilename, 100, 1, 1, new PostgresQueryPlanParser());
			dbDeployer = new PostgresDeployer(LogLevel.STATUS, fullDB, experimentCache, false);
			dbDesigner = new PostgresDesigner(LogLevel.STATUS, emptyDB, localPathToStatsFile, experimentCache);
			latencyMeter = new PostgresLatencyMeter(LogLevel.VERBOSE, true, fullDB, experimentCache, dbDeployer, dbDeployer, 10*60);
		}
		else {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length < 12 || args.length > 13) {
			log.error(String.valueOf(args.length));
		...
		try {
			...
			assert db_vendor.equals("vertica") || db_vendor.equals("microsoft") || db_vendor.equals("postgres");
			...
			} else if (db_vendor.equalsIgnoreCase("postgres")){
				System.out.print("In Postgres block of Cliffguard.java\n");
				allDatabaseConfigurations = DatabaseLoginConfiguration.loadDatabaseConfigurations(database_login_file, PostgresDatabaseLoginConfiguration.class.getSimpleName());
				DBVendor = PostgresDatabaseLoginConfiguration.class.getSimpleName();
			} else
			...
			//TODO: In this version, we do not check whether the two DB aliases have the same schema or not... We need to add this feature in the future.
			System.out.print("3\n");
